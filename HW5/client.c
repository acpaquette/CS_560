...
void three_a_plus_one_wrapper(void *number_ptr)
{
	...
	uint32_t n_num = htonl(number);
    write(client_socket, &n_num, sizeof(n_num));

    // get the result
    uint32_t n_result = 0;

    int read_count = recv(client_socket, &n_result, sizeof(n_result), MSG_WAITALL);

	int result = ntohl(n_result);
